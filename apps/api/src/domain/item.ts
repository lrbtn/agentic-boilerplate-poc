...
export class Item {
  ...
  withChanges(changes: { name?: string; quantity?: number; bought?: boolean }): Item {
    ...
    const bought = changes.bought === undefined ? this.bought : changes.bought;
    return new Item(this.id, name, quantity, bought, this.createdAt);
  }
}
